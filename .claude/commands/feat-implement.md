---
description: 기능 수정/추가 자율 파이프라인 (Phase 0~7, 규모 판단 + HITL 게이트 + 워크트리 격리)
...
- **Lint 정책 (효율)**: 중간 Phase(특히 Phase 5 quality-enforcer)에서 `pnpm -r lint`/`eslint --fix` 를 반복 실행하지 않는다. type-aware ESLint 는 1회 비용이 크고, 기계적 `eslint --fix`/`prettier` 는 **커밋 시 husky/lint-staged 가 1회 수행**하므로 중복이다. 각 agent 는 `tsc`(타입) + 의미론 검수에 집중한다. non-auto-fixable lint 에러는 발견 시점과 무관하게 수동 수정 비용이 동일하므로 커밋 단계에 위임한다.
- **husky pre-commit 실패 진단**: 커밋 시 husky/lint-staged 가 실패하면(lint-staged 는 파일명을 숨기므로 추측 금지), **즉시 변경 파일에 직접 `eslint` 를 돌려 `파일:줄:열` 을 특정**한 뒤 수정하고 재커밋한다. (워크스페이스별 eslint 설정 사용 — `apps/api/eslint.config.mjs` 등. `git diff --cached --name-only` 로 대상 파일 목록 확보.) 흔한 비자동수정 에러: inline `import('x').Type` 타입 주석(→ 상단 `import type` 으로) · `any` 값 구조분해(→ 명시 타입/`unknown` 캐스팅).

## Phase 0 전(前): 워크트리 격리 (동시 실행 대비 — 시작 시 1회 확인)

> **계획된 사용자 개입 지점이다** (자율 실행 규칙 4항(d)). 파이프라인의 **첫 동작**이며, 도메인 resolve(Phase 0)보다 먼저 수행한다.

여러 컨텍스트에서 feat-implement 를 **동시에 2건 이상** 실행하면 같은 워킹트리를 공유해 브랜치 전환·stash·lint-staged 가 경합한다 — onyu 도 husky `pre-commit` + `lint-staged`(`apps/api/src/**`, `apps/bot/src/**` 등 `eslint --fix`+`prettier`)가 구성돼 있어, 동시 커밋 시 변경 유실·타 커밋 혼입 위험이 있다. 이를 원천 차단하기 위해 **각 실행을 전용 git 워크트리 + 전용 브랜치에 격리**한다.

> **격리 단위는 "파이프라인 1건 전체"이지 "에이전트"가 아니다.** 개별 에이전트에는 `isolation:"worktree"` 옵션을 붙이지 않는다 — feat-implement 의 단계들은 독립 작업이 아니라 한 기능의 연속 단계라, 앞 단계 산출물(implementer)을 뒷 단계(quality-enforcer / tester / fe-tester)가 **같은 워킹트리에서 읽어야** 하기 때문이다. 에이전트별 임시 워크트리는 이 단계 간 데이터 흐름을 끊는다.
...
2. **제안 생성**: `$ARGUMENTS` 에서 추출한 기능 slug 로 다음을 제안한다.
   - 브랜치: `feature/<slug>` (변경 성격에 따라 `fix/` · `refactor/` · `chore/`) — **develop 에서 분기** (Git Workflow 규칙: "작업 브랜치는 항상 develop 에서 분기")
   - 워크트리 경로: `../onyu-<slug>` (메인 리포 형제 디렉토리)
