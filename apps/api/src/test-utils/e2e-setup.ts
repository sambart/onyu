/**
 * E2E 테스트 공통 셋업 (vitest setupFiles 용).
 *
 * testcontainers(PG15 + Redis7)를 1회 기동하고 env 를 주입한다.
 * integration-setup.ts 와 달리 마이그레이션을 1회만 실행하며,
 * 이 파일이 setupFiles 에 등록되어 각 worker(= 각 e2e 파일)가 컨테이너를 공유할 수 없으므로
 * 각 파일 단위로 컨테이너를 기동한다 (e2e 파일 수가 적으므로 수용 가능).
 */
