...
- **파일명 패턴**: `1777400000000-AdminUserTableInit` (타임스탬프: `1777400000000`)
...
- **파일명 패턴**: `1777400000001-AdminUserSeedSuperAdmin` (타임스탬프: `1777400000001`)
...
> **타임스탬프 근거**: 현재 최신 마이그레이션이 `1777300000000-AddSuperAdminAuditLog`이므로, 두 마이그레이션에 `1777400000000`, `1777400000001`을 각각 부여한다.

### 마이그레이션 실행 순서

```
1777300000000-AddSuperAdminAuditLog   (기존 — 이미 적용)
1777400000000-AdminUserTableInit      (신규 1 — 테이블/인덱스 생성)
1777400000001-AdminUserSeedSuperAdmin (신규 2 — 초기 데이터 삽입)
```
...
3. `pnpm --filter @onyu/api migration:generate -- --name AdminUserTableInit` 실행 → 생성된 파일 타임스탬프를 `1777400000000`으로 수정
4. `admin_user_seed_super_admin` 마이그레이션은 자동 생성 불가 — 수동으로 `1777400000001-AdminUserSeedSuperAdmin.ts` 작성 (INSERT/DELETE 쌍). up()의 INSERT는 반드시 `ON CONFLICT ("discordUserId") DO NOTHING`을 포함하여 멱등성 확보
