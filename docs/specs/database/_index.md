...
├────────────────────────────────────────────────────────────┤
│ PK id (uuid)                                                │
│ adminDiscordUserId (varchar, NOT NULL)                      │
│ guildId (varchar, NULLABLE)                                 │
│ httpMethod (varchar, NOT NULL)                              │
│ requestPath (varchar, NOT NULL)                             │
│ createdAt (timestamptz, NOT NULL, DEFAULT now())            │
└────────────────────────────────────────────────────────────┘
  (독립 테이블 — FK 없음, Discord ID 직접 저장)
  IDX_audit_log_admin: IDX(adminDiscordUserId)
  IDX_audit_log_guild: IDX(guildId)
  IDX_audit_log_created_at: IDX(createdAt DESC)

┌────────────────────────────────────────────────────────────┐
│      AdminUser (admin_user)                                 │
│      Super Admin — F-SUPER-ADMIN-007                        │
├────────────────────────────────────────────────────────────┤
│ PK id (uuid)                                                │
│ discordUserId (varchar, NOT NULL, UNIQUE)                   │
│ role (varchar, NOT NULL)                                    │
│   값: 'super_admin' | 'bot_operator'                        │
...
└────────────────────────────────────────────────────────────┘
  (독립 테이블 — FK 없음, Discord ID 직접 저장)
  UQ_admin_user_discord: UNIQUE(discordUserId)
```
...
## 슈퍼 관리자 (Super Admin) 도메인

### AuditLog (`audit_log`) — F-SUPER-ADMIN-006

> 플랫폼 관리자(`admin_user` 테이블 소속)가 발생시킨 API 요청을 자동 기록하는 감사 로그 테이블.
> `AuditLogInterceptor` (NestJS Interceptor)가 `/api/guilds/:guildId/*` 및 `/api/admin/*` 요청마다 INSERT를 수행한다.
> 관리자 식별은 DB 기반(`admin_user.role` + JWT payload)으로 전환됨 — 환경변수 `SUPER_ADMIN_IDS` 방식 대체.

| 컬럼 | 타입 | 제약조건 | 기본값 | 설명 |
|------|------|----------|--------|------|
| `id` | `uuid` | PK | `gen_random_uuid()` | 내부 PK. 시계열 삽입이 아닌 랜덤 분산이 필요하므로 uuid 사용 |
| `adminDiscordUserId` | `varchar` | NOT NULL | — | 열람한 슈퍼 관리자의 Discord user ID |
| `guildId` | `varchar` | NULLABLE | `NULL` | 열람 대상 길드 ID. `/api/admin/guilds` 등 길드 비특정 엔드포인트는 NULL |
| `httpMethod` | `varchar(10)` | NOT NULL | — | HTTP 메서드 (`GET`, `POST` 등) |
| `requestPath` | `varchar(500)` | NOT NULL | — | 요청 경로 (예: `/api/guilds/123456789/voice/stats`) |
| `createdAt` | `timestamptz` | NOT NULL | `now()` | 요청 시각 (UTC, timezone-aware) |

- **스키마**: `public`
- **관계**: 독립 테이블 — 타 테이블과 FK 관계 없음. Discord ID 직접 저장
- **파일 (예정)**: `apps/api/src/admin/audit/audit-log.orm-entity.ts`

#### 설계 결정 사항

| 항목 | 결정 | 근거 |
|------|------|------|
| PK 타입 | `uuid` (gen_random_uuid) | 감사 로그는 외부에서 ID 예측을 막는 것이 바람직하며, BIGSERIAL 순번 노출 없이 고유성 보장 |
| `timestamp` vs `timestamptz` | `timestamptz` | 감사 로그는 절대 시각이 중요. 서버 타임존(Asia/Seoul)과 무관하게 UTC 기준으로 저장·조회 가능 |
| `httpMethod` | `varchar(10)` | `GET`/`POST`/`PUT`/`PATCH`/`DELETE` 범위. enum 대신 varchar — 추후 메서드 확장 시 마이그레이션 불필요 |
| `requestPath` | `varchar(500)` | Discord ID 등 포함 경로가 길어질 수 있음. text 대신 varchar(500)으로 인덱스 부하 절감 |
| 타 테이블 FK 배제 | FK 없음 | 감사 로그는 원본 레코드 삭제 후에도 이력 보존 필요. FK 제약 시 CASCADE DELETE 위험 |

#### 인덱스

| 인덱스 | 컬럼 | 종류 | 용도 |
|--------|------|------|------|
| `IDX_audit_log_admin` | `(adminDiscordUserId)` | IDX | 관리자별 열람 이력 조회 |
| `IDX_audit_log_guild` | `(guildId)` | IDX | 길드별 접근 이력 조회. NULL 포함 — 전체 조회 포함 |
| `IDX_audit_log_created_at` | `(createdAt DESC)` | IDX | 최신순 시계열 조회 (기본 정렬 패턴) |

> 복합 인덱스는 현재 단계에서 추가하지 않는다. 감사 로그 조회 UI는 Phase 1 범위 외(PRD "범위 제외" 섹션 참조)이며, 단순 시계열·관리자·길드 단위 조회만 발생한다. 과설계 방지 원칙에 따라 단일 컬럼 인덱스 3개로 충분하다.

#### 데이터 보존 정책

별도 보존 기간 미정 (PRD 명시 없음). 초기에는 무기한 보존하며, 향후 데이터 볼륨 증가 시 TTL 정책을 별도 티켓으로 결정한다.
...
CREATE INDEX "IDX_audit_log_guild"      ON "audit_log" ("guildId");
...
### `1777300000000-AddSuperAdminAuditLog`

> 신규 테이블 CREATE 전용. 파괴적 변경(DROP/DELETE/컬럼 제거) 없음.

**배경**: 슈퍼 관리자 콘솔 도입(F-SUPER-ADMIN-006)으로 감사 로그 저장이 필요해졌다. 슈퍼 관리자가 발생시킨 API 요청마다 `AuditLogInterceptor`가 `audit_log` 테이블에 INSERT한다.

**선행 조건**:
- `apps/api/src/admin/audit/audit-log.orm-entity.ts` 엔티티 파일 작성 완료
- `apps/api/src/admin/admin.module.ts` 에 `AuditLog` 엔티티 등록 완료

**마이그레이션 파일 경로**: `apps/api/src/migrations/1777300000000-AddSuperAdminAuditLog.ts`

**타임스탬프**: `1777300000000` (기존 최신 마이그레이션 `1777200000000-DropGuildCoPresenceConfig` 보다 큰 값)
...
### AdminUser (`admin_user`) — F-SUPER-ADMIN-007

> 플랫폼 관리자(super_admin, bot_operator) 정보를 DB에 영속화한다.
> Discord OAuth 로그인 시 `createToken()`이 이 테이블을 조회하여 JWT payload에 `role` + `scopes`를 포함시킨다.
> 환경변수 `SUPER_ADMIN_IDS` 방식을 대체한다.

| 컬럼 | 타입 | 제약조건 | 기본값 | 설명 |
|------|------|----------|--------|------|
| `id` | `uuid` | PK | `gen_random_uuid()` | 내부 PK. audit_log와 동일하게 uuid 사용 |
| `discordUserId` | `varchar` | NOT NULL, UNIQUE | — | Discord user ID. 로그인 시 `findByDiscordId` 조회 키 |
| `role` | `varchar` | NOT NULL | — | 관리자 역할. `'super_admin'` (전체 scope) \| `'bot_operator'` (`admin:manage` 제외) |
| `permissions` | `text[]` | NULLABLE | `NULL` | role 기본 scope 오버라이드용. 초기 미사용. 향후 "이 운영자는 특정 scope만" 세분화 시 활용. **NULL = role 기본 scope 사용. `[]`(빈 배열) = scope 없음(전체 차단)** — 앱 레이어에서 두 값을 구분하여 처리해야 한다 |
| `grantedBy` | `varchar` | NULLABLE | `NULL` | 부여자 Discord user ID (감사 목적). seed 삽입 시 `'seed'` |
| `isActive` | `boolean` | NOT NULL | `true` | `false`이면 로그인 시 `role: null`, `scopes: []` 처리 (물리 삭제 없이 비활성화) |
| `createdAt` | `timestamptz` | NOT NULL | `now()` | 레코드 생성 시각 (UTC, timezone-aware) |
| `updatedAt` | `timestamptz` | NOT NULL | `now()` | 레코드 최종 갱신 시각 (UTC, timezone-aware) |

- **스키마**: `public`
- **관계**: 독립 테이블 — 타 테이블과 FK 관계 없음. Discord ID 직접 저장
- **파일 (신규)**: `apps/api/src/super-admin/infrastructure/admin-user.orm-entity.ts`
- **인프라 패턴**: `audit-log.orm-entity.ts` 복제 (`@Entity({ name: 'admin_user', schema: 'public' })`, `@PrimaryGeneratedColumn('uuid')`, `timestamptz`)

#### 설계 결정 사항

| 항목 | 결정 | 근거 |
|------|------|------|
| PK 타입 | `uuid` (`PrimaryGeneratedColumn('uuid')`) | audit_log 패턴 통일. 외부 노출 ID 예측 불가 |
| `timestamp` vs `timestamptz` | `timestamptz` | 권한 부여/변경 시각은 절대 시각 기록이 중요. audit_log 동일 패턴 |
| `role` 타입 | `varchar` (enum/CHECK 아님) | 향후 역할 추가 시 마이그레이션 불필요. 유효값 검증은 애플리케이션 레이어(DTO class-validator + AdminUserService)에서 수행. CHECK constraint 도입 검토 시: `ADD CONSTRAINT ck_admin_user_role CHECK (role IN ('super_admin', 'bot_operator'))` — 비파괴적이지만 역할 추가 시마다 마이그레이션 필요하므로 현 단계에서 보류 |
| `permissions` 타입 | `text[]` | PostgreSQL 네이티브 배열. 초기 미사용이지만 컬럼 예약으로 향후 ALTER 없이 세분화 가능. NULL = role 기본 scope, `[]`(빈 배열) = scope 전체 차단 — 앱 레이어에서 두 경우를 구분해야 함 |
| FK 배제 | FK 없음 | 관리자 삭제(비활성화)가 감사 로그 등 타 이력과 충돌하지 않도록 Discord ID 직접 저장 |
| `isActive` 방식 | 물리 삭제 없이 `false` 처리 | 마지막 `super_admin` 보호 및 권한 회수 이력 보존 |
| 자기 비활성화 방지 | 앱 레이어(AdminUserService) | DB 트리거는 TypeORM 관리 비용 증가. `req.user.discordUserId === target.discordUserId` 체크를 서비스 레이어에서 수행 |
| 최소 1명 super_admin 유지 | 앱 레이어(AdminUserService) | 비활성화 / 역할 다운그레이드 전 `COUNT(role='super_admin' AND isActive=true) > 1` 체크를 서비스 레이어에서 수행. DB defer 제약은 오버엔지니어링 |

#### 인덱스

| 인덱스 | 컬럼 | 종류 | 용도 |
|--------|------|------|------|
| `UQ_admin_user_discord` | `(discordUserId)` | UNIQUE | 로그인 시 `findByDiscordId` — `createToken()`에서 매 로그인 1회 조회. UNIQUE 제약으로 중복 INSERT 방지 |

#### role → scope 기본 매핑 (참조용)

| role | JWT scopes[] 기본값 |
|------|---------------------|
| `super_admin` | `guild:view`, `admin:manage`, `guild:manage`, `billing:manage`, `churn:view`, `usage:view`, `onboarding:view`, `notification:manage`, `feature-flag:manage` |
| `bot_operator` | `guild:view`, `guild:manage`, `billing:manage`, `churn:view`, `usage:view`, `onboarding:view`, `notification:manage`, `feature-flag:manage` |

> scope 매핑은 DB 컬럼이 아닌 애플리케이션 레이어(`AdminUserService`)에서 `role` 값 기반으로 산출된다. `permissions` 컬럼이 NOT NULL 배열이면 기본 scope를 override한다.
...
COMMENT ON TABLE "admin_user" IS '플랫폼 관리자 역할·권한 영속화 (F-SUPER-ADMIN-007)';
COMMENT ON COLUMN "admin_user"."discordUserId" IS 'Discord user ID (로그인 조회 키)';
COMMENT ON COLUMN "admin_user"."role" IS '관리자 역할: super_admin | bot_operator';
COMMENT ON COLUMN "admin_user"."permissions" IS 'role 기본 scope 오버라이드용. NULL이면 role 기본값 사용';
COMMENT ON COLUMN "admin_user"."grantedBy" IS '부여자 Discord user ID. seed 삽입 시 ''seed''';
COMMENT ON COLUMN "admin_user"."isActive" IS 'false이면 로그인 시 role null 처리 (물리 삭제 없이 비활성화)';

-- 2. 최초 슈퍼관리자 seed INSERT (멱등 — 중복 실행 시 SKIP)
...
> 본 섹션은 implementer(`apps/api/src/super-admin/infrastructure/admin-user.orm-entity.ts` 작성 + 마이그레이션 생성/실행)가 참조하는 진실 소스다.
...
| 구분 | 대상 | 변경 내용 |
|------|------|-----------|
| 신규 테이블 | `admin_user` | F-SUPER-ADMIN-007 명세대로 신규 CREATE |
| 신규 인덱스 | `UQ_admin_user_discord` | `admin_user.discordUserId` UNIQUE 인덱스 신규 추가 |
| 신규 seed 데이터 | `admin_user` 레코드 1건 | 최초 슈퍼관리자 INSERT (SeedInitialSuperAdmin) |
| 기존 테이블 변경 | 없음 | 기존 테이블/컬럼/인덱스 변경 없음 |
| 파괴적 변경 | 없음 | DROP TABLE / DROP COLUMN / 대량 DELETE 없음 |

### 신규 테이블 상세 — `admin_user`

| 컬럼 | PostgreSQL 타입 | nullable | unique | 기본값 | 인덱스 |
|------|-----------------|----------|--------|--------|--------|
| `id` | `uuid` | NOT NULL | — | `gen_random_uuid()` | PK |
| `discordUserId` | `varchar` | NOT NULL | UNIQUE | — | `UQ_admin_user_discord` |
| `role` | `varchar` | NOT NULL | — | — | — |
| `permissions` | `text[]` | NULLABLE | — | `NULL` | — |
| `grantedBy` | `varchar` | NULLABLE | — | `NULL` | — |
| `isActive` | `boolean` | NOT NULL | — | `true` | — |
| `createdAt` | `timestamptz` | NOT NULL | — | `now()` | — |
| `updatedAt` | `timestamptz` | NOT NULL | — | `now()` | — |

**TypeORM 데코레이터 지침** (`audit-log.orm-entity.ts` 패턴 복제):
...
#### 1. `admin_user_table_init`

- **파일명 패턴**: `1777500000000-AdminUserTableInit` (타임스탬프: `1777500000000`)
- **내용**: `admin_user` 테이블 CREATE + `UQ_admin_user_discord` UNIQUE 인덱스 생성
- **up()**: CREATE TABLE + CREATE UNIQUE INDEX + COMMENT ON
- **down()**: DROP INDEX + DROP TABLE
- **파괴적 변경**: 없음 (신규 테이블 추가만)

#### 2. `admin_user_seed_super_admin`

- **파일명 패턴**: `1777500000001-AdminUserSeedSuperAdmin` (타임스탬프: `1777500000001`)
- **내용**: 최초 슈퍼관리자 1명 INSERT (discordUserId=`383635512252039168`, role=`super_admin`, grantedBy=`seed`)
- **up()**: `INSERT INTO admin_user ... ON CONFLICT ("discordUserId") DO NOTHING` — 멱등 보장. 동일 discordUserId가 이미 존재하면 SKIP (중복 실행 시 unique violation 방지)
- **down()**: `DELETE FROM admin_user WHERE "discordUserId" = '383635512252039168' AND "grantedBy" = 'seed'` — seed row만 제거. 수동 편집된 동일 discordUserId 레코드는 grantedBy가 다르므로 보호됨. 테이블/컬럼 DROP 없음, 파괴적 변경 아님.
- **파괴적 변경**: 없음

> **타임스탬프 근거**: develop의 `1777400000000-AddRolePanel`과 타임스탬프 충돌을 피하기 위해 두 마이그레이션에 `1777500000000`, `1777500000001`을 각각 부여한다.

### 마이그레이션 실행 순서

```
1777300000000-AddSuperAdminAuditLog   (기존 — 이미 적용)
1777400000000-AddRolePanel            (develop 머지분)
1777500000000-AdminUserTableInit      (신규 1 — 테이블/인덱스 생성)
1777500000001-AdminUserSeedSuperAdmin (신규 2 — 초기 데이터 삽입)
```

### 구현 지침 (implementer 참조)

1. `apps/api/src/super-admin/infrastructure/admin-user.orm-entity.ts` 작성 — 위 TypeORM 데코레이터 지침 그대로 복제
2. `apps/api/src/super-admin/super-admin.module.ts` 에 `AdminUserOrmEntity` 등록 (`TypeOrmModule.forFeature([AdminUserOrmEntity])`)
3. `pnpm --filter @onyu/api migration:generate -- --name AdminUserTableInit` 실행 → 생성된 파일 타임스탬프를 `1777500000000`으로 수정 (develop의 `1777400000000-AddRolePanel`과 충돌 회피)
4. `admin_user_seed_super_admin` 마이그레이션은 자동 생성 불가 — 수동으로 `1777500000001-AdminUserSeedSuperAdmin.ts` 작성 (INSERT/DELETE 쌍). up()의 INSERT는 반드시 `ON CONFLICT ("discordUserId") DO NOTHING`을 포함하여 멱등성 확보
5. `pnpm --filter @onyu/api migration:run` 실행하여 두 마이그레이션 순차 적용
