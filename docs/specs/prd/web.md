...
| AI 진단 텍스트 | LLM이 생성한 서버 건강도 종합 진단 텍스트. 점수 산출 근거, 주요 이슈, 개선 방향을 포함. 로딩 중 스켈레톤 표시 |
...
| "분석 새로고침" 버튼 | 클릭 시 `POST /api/guilds/{guildId}/voice-analytics/ai-insight` 호출하여 LLM 재분석 실행. 쿨다운 10분 적용 (Redis 기반). 쿨다운 중에는 버튼 비활성화 및 남은 시간 표시 |
| 마지막 분석 시각 표시 | LLM 마지막 호출 시각을 "N분 전" 형태로 표시 |

#### 초기 로드 동작

1. 페이지 마운트 시 기본 기간(30일)으로 섹션 1~4 데이터를 병렬 조회
2. AI 인사이트(섹션 5)는 캐시된 결과가 있으면 즉시 표시, 없으면 자동으로 LLM 호출 실행
3. 기간 변경 시 섹션 1~4 데이터를 재조회. AI 인사이트는 기간 변경에 따라 함께 재조회하되 쿨다운과 무관하게 갱신 (기간이 달라지면 새 분석으로 간주)

#### 호출 API

| 메서드 | 경로 | 설명 |
|--------|------|------|
| `GET` | `/api/guilds/{guildId}/voice-analytics/health-score?days=N` | 건강도 점수 + AI 진단 텍스트 조회. LLM 생성. 응답: `{ score, prevScore, delta, diagnosis }` |
