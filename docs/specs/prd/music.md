...
디스코드 음성 채널에서 음악을 재생하는 기능을 제공한다. Lavalink v4(Java 기반 오디오 노드)와 Kazagumo v3(Shoukaku v4 래퍼) 조합을 사용한다. 검색어 또는 Spotify/Deezer URL 입력을 지원하며, **Spotify를 메타데이터 검색 소스**로, **Deezer를 실제 스트리밍 소스(128kbps MP3)**로 활용한다. YouTube 의존성은 완전히 제거되었다. Lavalink는 별도 Docker 컨테이너로 실행되며, LavaSrc 플러그인(`lavasrc-plugin:4.3.0`)이 Spotify·Deezer 연동을 담당한다.
...
[Lavalink v4 컨테이너]        ← Java 기반 Lavaplayer 오디오 처리
    │
    ▼
[Spotify (검색/메타데이터) → Deezer (스트리밍)]  ← LavaSrc 플러그인 처리
```

**검색 해석 흐름**:

1. 검색어 입력 → Kazagumo가 `spsearch:검색어`로 변환 (`defaultSearchEngine: spotify`)
2. LavaSrc가 Spotify API에서 메타데이터(제목, 아티스트, ISRC, 앨범아트) 조회
3. `providers` 순서대로 실제 스트리밍 소스 탐색:
   - `dzisrc:%ISRC%` → Deezer에서 ISRC로 정확 매칭 (우선 시도)
...
- **입력**: 텍스트 검색어, Spotify URL (트랙/플레이리스트/앨범), Deezer URL (트랙)
- **동작**:
  1. 유저가 음성 채널에 접속해 있는지 확인 (미접속 시 에러 응답, ephemeral)
  2. Kazagumo를 통해 Lavalink에 트랙 또는 플레이리스트 검색 요청
     - 텍스트 검색어 → `spsearch:검색어` (Spotify 검색 → Deezer 스트리밍)
     - Spotify/Deezer URL → LavaSrc가 직접 처리
...
- **지원 소스**: 텍스트 검색어 (Spotify 검색 → Deezer 스트리밍), Spotify URL (트랙/플레이리스트/앨범), Deezer URL (트랙)
- **미지원 소스**: YouTube URL (YouTube 의존성 완전 제거됨)
...
| 플러그인 | 버전 | 역할 |
|----------|------|------|
| `com.github.topi314.lavasrc:lavasrc-plugin` | `4.3.0` | Spotify 메타데이터 검색 + Deezer 스트리밍 연동 |

- YouTube 플러그인(`dev.lavalink.youtube:youtube-plugin`) **제거됨**
- LavaSrc providers 순서: `dzisrc:%ISRC%` (ISRC 정확 매칭) → `dzsearch:%QUERY%` (텍스트 fallback)
...
| `SPOTIFY_CLIENT_ID` | Spotify Developer App Client ID | (Spotify Dashboard에서 발급) |
| `SPOTIFY_CLIENT_SECRET` | Spotify Developer App Client Secret | (Spotify Dashboard에서 발급) |
...
**Lavalink 플러그인 변경**:
- 제거: `dev.lavalink.youtube:youtube-plugin:1.18.0`
- 추가: `com.github.topi314.lavasrc:lavasrc-plugin:4.3.0`
